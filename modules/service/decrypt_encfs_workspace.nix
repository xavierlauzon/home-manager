 ...
 config =
   let
     mkStartScript = name: pkgs.writeShellScript "${name}.sh" ''
       #set -uo pipefail
       if [ -d $HOME/Nextcloud/xl/.encfs ] ; then
         if ${pkgs.gnugrep}/bin/grep -qs "$HOME/Documents/Workspace" /proc/mounts; then
           ...
           if [ -f "$XDG_RUNTIME_DIR/secrets/encfs/workspace" ] ; then
             ...
             ${pkgs.coreutils}/bin/cat "$XDG_RUNTIME_DIR/secrets/encfs/workspace" | /run/wrappers/bin/sudo ${pkgs.encfs}/bin/encfs --public -S -f "$HOME/Nextcloud/xl/.encfs" "$HOME/Documents/Workspace"
           else
   ...
   mkIf cfg.enable {
     ...
     sops.secrets = {
       "encfs/workspace" = {
         sopsFile =  ../../home/xl/secrets/encfs_workspace.yaml ;
       };
     };
  };
}
